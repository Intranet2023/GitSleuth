...
def process_search_results(search_results, all_data, query, headers, group_name, ignored_filenames):
    ...
    for item in search_results['items']:
        if item['path'] not in ignored_filenames:
            ...
            try:
                file_contents = GitSleuth_API_orig.get_file_contents(repo_name, file_path, headers)
                if file_contents:
...
def perform_grouped_searches(domain):
    ...
    for group_name in selected_groups:
        ...
        for query in queries:
            ...
            search_results = GitSleuth_API_orig.search_github_code(query, headers)
...
def check_and_handle_rate_limit(headers):
    ...
    """
    remaining_limit = GitSleuth_API_orig.check_rate_limit(headers)
...
def perform_custom_search(domain):
    ...
    config = load_config()
    headers = get_headers(config)
    search_results = GitSleuth_API_orig.search_github_code(full_query, headers)
    all_data = []
    if search_results and 'items' in search_results:
        for item in search_results['items']:
            repo_name = item['repository']['full_name']

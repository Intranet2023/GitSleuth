...
def check_rate_limit(headers):
    ...
    headers (dict): Headers for the GitHub API request.
    Returns:
    tuple: A tuple containing the number of requests remaining and the reset timestamp.
    """
    ...
    if response.status_code == 200:
        ...
        return core['remaining'], core['reset'], search['remaining'], search['reset']
    else:
        logging.error(f"Failed to fetch rate limit status: {response.status_code} {response.text}")
        return 0, None, 0, None

def search_github_code(query, headers):
    logging.info(f"Checking rate limit status before searching for query: {query}")
    core_remaining, core_reset, search_remaining, search_reset = check_rate_limit(headers)

    if search_remaining > 0:
        search_url = f"{GITHUB_API_URL}search/code?q={query}"
        logging.info(f"Sending API request to search code: {search_url}")
        response = requests.get(search_url, headers=headers)
        return handle_api_response(response)
    else:
        wait_time = max(0, search_reset - int(time.time())) + 10
        logging.warning(f"Search API rate limit reached. Waiting for {wait_time} seconds until reset.")
        time.sleep(wait_time)
        return search_github_code(query, headers)
...
def fetch_repository_data(repo_name, headers):
    """
    Fetches and aggregates important data from a specific GitHub repository.
    Parameters:
    repo_name (str): The full name of the repository (username/repo).
    headers (dict): Headers to be used for the GitHub API request.
    Returns:
    dict: A dictionary containing aggregated data from the repository.
    """
    repo_info = get_repo_info(repo_name, headers)
    commits = get_commit_history(repo_name, headers)
    readme_content = get_readme_contents(repo_name, headers)
    ...
    return {
        "repo_name": repo_name,
        "owner": repo_info['owner']['login'] if repo_info else None,
        "repo_creation_date": repo_info['created_at'] if repo_info else None,
        "last_commit_date": last_commit_date,
        "first_commit_date": first_commit_date,
        "readme_contents": readme_content
    }
